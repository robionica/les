...
from les.ext.coin import osi_sym_solver_interface
...
class OsiSymSolver(osi_sym_solver_interface.OsiSymSolverInterface, BILPSolverBase):
  """This class represents OSI Solver Interface for SYMPHONY."""

  def __init__(self):
    BILPSolverBase.__init__(self)
    osi_sym_solver_interface.OsiSymSolverInterface.__init__(self)
    self._problem = None
  ...
  def load_problem(self, problem, details={}):
    ...
    if details.get("obj_coefs", True):
      ...
      # fault.
      if problem.get_num_variables() > 1:
        ...
        col = coin_utils.CoinPackedVector()
        self.add_col(col, 0., 1., problem.get_objective().values()[0].astype(float))
        self.set_integer(1)
